            ...
            <div class="chat-history-body bg-body">
              <ul class="list-unstyled chat-history">
                <div class="mt-4" v-for="(item, index) in ticket" :key="index">
                  <li v-if="item.SupporterId != null" class="chat-message chat-message-right" style="text-align: right">
                    <div class="d-flex overflow-hidden">
                      <div class="chat-message-wrapper flex-grow-1 w-50">
                        <div class="chat-message-text" style="
                          ...
                          <div v-for="(
                              attachedFile, fileIndex
                            ) in item.Attachments" :key="fileIndex">
                            <a target="blank" :href="attachedFile.FilePath" style="color: #575b58">دانلود پیوست {{
                              (fileIndex += 1) }}</a>
                          </div>
                        </div>
                      ...
                      <div class="chat-message-wrapper flex-grow-1 w-50">
                        <div class="chat-message-text chat-background" style="text-align: right">
                          ...
                          <div v-for="(
                            ...
                            <a target="blank" :href="attachedFile.FilePath" style="color: #2f5233">دانلود پیوست {{
                              (fileIndex += 1) }}</a>
                          </div>
                        </div>
...
export default {
  ...
  methods: {
    ...
    getMessages: function () {
      ...
      $this.loading = true;
      axios.panelUrl
        .get(`/v1/Tickets/Ticket/Find/${this.id}`)
    ...
    sendMessage: function () {
      ...
      $this.loading = true;
      axios.panelUrl
        .post("/v1/Tickets/Ticket/AddTicketByUser", {
          Title: $this.title,
          TicketId: $this.id,
